...
int last_seq_written=0;
int last_seq_read=0;
int next_seq_expected=0;
int discard;
...
void sigPollNoRetransmitHandler(int sig){
    int pid = (int)getpid();
    int bytes,seq_num;
    ...
    memset(audio_buffer.packets[audio_buffer.writePosition].array,'\0',payload_size+5);
    strncpy(audio_buffer.packets[audio_buffer.writePosition].array,&read_buffer[0],payload_size+5);
    /***************************************/
...
void sigPollRetransmitHandler(int sig){
    int pid = (int)getpid();
    int bytes,seq_num;
    char read_buffer[payload_size+5], write_buffer[500];
    struct sockaddr_in server_addr;
    ...
    // recvfrom server
    while(1){
        bytes = recvfrom(udp_client_socket,read_buffer,payload_size+5,0,(struct sockaddr*) &server_addr, &addr_len);
        if(bytes==-1){
        ...
        if (temp!='\0')
            sscanf(temp, "%d", &seq_num);
        /****************/
        //printf("Received seq Number %d", seq_num);
        /*******************/
 
        current_packet++;
        if(current_packet==discard){
            if(audiobuf_pos[seq_num]==-1){
                /*************/
                //printf("New packet to discard %d\n",seq_num);
                /*******************/
                int limit;
                int ret = sem_getvalue(&audio_buffer.validItems,&limit);
                if (limit > arr_size-1){
                    /***************/
                    //printf("Not discarding packet. Buffer full\n");
                    /********************/
                    last_seq_written = seq_num;
                    next_seq_expected = (last_seq_written+1)%10000;
                    //printf("Buffer is full. limit is %d\n", limit);
                    continue;
                }
                audiobuf_pos[next_seq_expected]=audio_buffer.writePosition;
                snprintf(read_buffer,payload_size+5,"%04d%s",next_seq_expected,"$");
                strncpy(audio_buffer.packets[audio_buffer.writePosition].array,read_buffer,payload_size+5);
                audio_buffer.writePosition = (audio_buffer.writePosition+1)%arr_size;
                /********************/
                //printf("seq_num %d next_seq_expected %d writePosition %d audio_buf position %d\n",seq_num, next_seq_expected,audio_buffer.writePosition,audiobuf_pos[next_seq_expected]);
                /************************/
            }
            else{
                /*******************/
                //printf("Discarding old packet %d", seq_num);
                /*********************/
                audiobuf_pos[seq_num]=-1;
            }
            current_packet=0;
        }
        else{
            if(audiobuf_pos[seq_num]!=-1){
                /***********************/
                //printf("Writing retransmitted packet seq_num %d, audiobuf_pos %d audio_buffer %s\n",seq_num, audiobuf_pos[seq_num], audio_buffer.packets[audiobuf_pos[seq_num]].array);
                /**********************/
                char p[5];
                memset(p,'\0',5);
                strncpy(p,audio_buffer.packets[audiobuf_pos[seq_num]].array ,4);
                int current_seq;
                sscanf(p,"%d",&current_seq);
                if(seq_num==current_seq){
                    if((last_seq_written<last_seq_read && (seq_num>last_seq_read || seq_num<last_seq_written)) || (last_seq_written>last_seq_read && (seq_num>last_seq_read && seq_num<last_seq_written))){
                        memset(audio_buffer.packets[audiobuf_pos[seq_num]].array,'\0',payload_size+5);
                        strncpy(audio_buffer.packets[audiobuf_pos[seq_num]].array,read_buffer,payload_size+5);
                        int limit, ret;
                        sem_post(&audio_buffer.validItems);
                        ret = sem_getvalue(&audio_buffer.validItems,&limit);
                        /***********************************/
                ...
                if(seq_num!=next_seq_expected){
                    memset(write_buffer,'\0',500);
                    snprintf(write_buffer,500,"%s%04d","M",next_seq_expected);
                    /************************/
                    //printf("Sending retransmisison request seq_num %d, next_expected_seq %d\n",seq_num, next_seq_expected);
                    /*****************************/
                    if(sendto(udp_client_socket, write_buffer, strlen(write_buffer), 0, (struct sockaddr*)&udp_server_addr, sizeof(udp_server_addr))==-1){
                        printf("Error sending feedback to server %s\n", strerror(errno));
                        exit(1);
...
int main(int argc, char** argv){
    ...
    if (argc<13){
        printf("Format is audiolisten server-ip server-tcp-port client-udp-port payload-size playback_del gamma buf-sz target-buf logfile-c filename k retransmission");
        exit(1);
    }
    ...
    discard = atoi(argv[11]);
    retransmission = atoi(argv[12]);

    audiobuf_pos = (int*)malloc(10000*sizeof(int));
    int v=0;
    for(;v<10000;v++){
        audiobuf_pos[v]=-1;
    }
    ...
    for(k=0;k<arr_size;k++){
        audio_buffer.packets[k].array = (char*)malloc(sizeof(char)*(payload_size+5));
        memset(audio_buffer.packets[k].array,'\0',payload_size+5);
    }
    ...
    if(retransmission){
        sigPoll.sa_handler = sigPollRetransmitHandler;
    }
    else{
        sigPoll.sa_handler = sigPollNoRetransmitHandler;
    }
   ...
   if (strcmp(temp,"OK")!=0){
    ...
    while(1) {
        ...
        if(retransmission){
            while(strlen(audio_buffer.packets[audio_buffer.readPosition].array)==5 && audio_buffer.packets[audio_buffer.readPosition].array[4]=='$'){
                /**************************/
                //printf("Not reading %d. Array %s\n",audio_buffer.readPosition,audio_buffer.packets[audio_buffer.readPosition].array);
                /****************************/
                audio_buffer.readPosition = (audio_buffer.readPosition+1)%arr_size;
            }
        }
        if((bytes=write(audio_fd,&(audio_buffer.packets[audio_buffer.readPosition].array[4]),strlen(&(audio_buffer.packets[audio_buffer.readPosition].array[4]))))==-1){
        ...
        if(retransmission){
            char temp[5];
            memset(temp,'\0',5);
            strncpy(temp,audio_buffer.packets[audio_buffer.readPosition].array,4);
            sscanf(temp,"%d",&last_seq_read);
            /***********************/
            //printf("Last read seq %d\n", last_seq_read);
            /*************************/
        }
        /***************************************/
