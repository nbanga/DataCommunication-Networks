...
void sigPollHandler(int sig){
    ...
    if((bytes = recvfrom(sock_fd,read_buffer,MAX_BUF,0,(struct sockaddr*) &client_addr, &addr_len))==-1){
    ...
    if(fprintf(in_stream,"%lf%s%lf\n",(float)(timestamp.tv_sec-start.tv_sec)+(float)(timestamp.tv_usec-start.tv_usec)/1000000.0,";",lambda)<0){
        printf("Error in fprintf to in_stream %s",strerror(errno));
        exit(1);
    }
    //printf("%d Lambda - %lf packet_spacing - %lf\n", pid, lambda, packet_spacing);
}
...
int main(int argc, char** argv){
    ...
    if (argc<7){
        printf("Format is audiostreamd tcp-port udp-port payload-size packet-spacing mode logfile-s");
        exit(1);
    }
