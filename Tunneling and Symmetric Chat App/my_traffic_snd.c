...
int main(int argc, char** argv){
    ...
    if (argc<8) {
        printf("Format is 'traffic_snd vpn-IP-address vpn-port-number server-IP server-IP-port payload-size packet-count packet-spacing '\n");
        exit(1);
    }
    ...
    client_addr.sin_addr.s_addr = inet_addr("128.10.2.13"); 
    ...
    // create packet to be sent to tunnel
    char write_buffer[MAX_BUF], read_buffer[MAX_BUF];
    ...
    // receive response form tunnel
    if(recvfrom(sd_client,read_buffer,MAX_BUF,0,(struct sockaddr *)&server_addr_ret, &addr_len_ret)==-1){
        printf("Erorr reading response form tunnel\n");
        exit(1);
    }
    printf("%s\n",read_buffer);
